<script>
  ...
  let posts = [
    { id: 1, topic: 'historias', tag: 'Historias', title: 'Cuando dejé de minimizar lo que sentía', body: 'Anotar cuándo aparecía la comezón me ayudó a explicarlo mejor en consulta.', author: 'Anónimo', time: 'Hace 2 h', votes: 48, comments: 12, liked: false, thread: [{ id: 11, author: 'Anónimo', body: 'Me pasó algo parecido. Llevar notas hizo más fácil explicar los días difíciles.', replies: [{ id: 111, author: 'Anónimo', body: 'Sí, escribirlo también me ayudó a ver patrones.' }] }, { id: 12, author: 'Anónimo', body: 'Gracias por compartirlo, no sabía por dónde empezar.', replies: [] }] },
    { id: 2, topic: 'consulta', tag: 'Preparar mi consulta', title: '¿Qué información les ha servido llevar?', body: 'Estoy preparando mi primera consulta con dermatología. ¿Qué preguntas les ayudaron?', author: 'Anónimo', time: 'Hace 5 h', votes: 31, comments: 9, liked: false, thread: [{ id: 21, author: 'Anónimo', body: 'Yo llevé fotos con fecha y una lista de lo que había cambiado.', replies: [] }] },
    { id: 3, topic: 'dia-a-dia', tag: 'Día a día', title: 'Encontrar pequeños espacios de calma', body: 'Compartir cómo me afecta en el sueño me hizo sentir menos solo y más preparado.', author: 'Anónimo', time: 'Ayer', votes: 76, comments: 18, liked: false, thread: [{ id: 31, author: 'Anónimo', body: 'Leer esto me hizo sentir acompañado. Gracias.', replies: [{ id: 311, author: 'Anónimo', body: 'Qué bueno que te sirvió leerlo.' }] }] }
  ]
  let activeTopic = 'todos', search = '', composerOpen = false, draftTitle = '', draft = '', openComments = {}, commentDrafts = {}, replyDrafts = {}, replyingTo = null
  ...
  function addComment(id) {
    ...
    if (!body) return
    posts = posts.map(post => post.id === id ? { ...post, comments: post.comments + 1, thread: [...(post.thread || []), { id: Date.now(), author: 'Anónimo', body, replies: [] }] } : post)
    commentDrafts = { ...commentDrafts, [id]: '' }
  }
  function addReply(postId, commentId) {
    const body = (replyDrafts[commentId] || '').trim()
    if (!body) return
    posts = posts.map(post => post.id === postId ? { ...post, comments: post.comments + 1, thread: post.thread.map(comment => comment.id === commentId ? { ...comment, replies: [...comment.replies, { id: Date.now(), author: 'Anónimo', body }] } : comment) } : post)
    replyDrafts = { ...replyDrafts, [commentId]: '' }
    replyingTo = null
  }
  function publish() {
    if (!draftTitle.trim() || !draft.trim()) return
    posts = [{ id: Date.now(), topic: 'historias', tag: 'Nueva historia', title: draftTitle.trim(), body: draft.trim(), author: 'Anónimo', time: 'Ahora', votes: 1, comments: 0, liked: true, thread: [] }, ...posts]
...
<section class="community" id="comunidad" aria-labelledby="community-title">
  ...
  {#if composerOpen}<div class="community-composer rise"><p class="composer-hint"><strong>Participación 100% anónima:</strong> Este espacio es seguro y anónimo. No solicitamos ni recopilamos nombres ni datos personales.</p><label for="community-title-draft">Título de tu historia</label><input id="community-title-draft" class="composer-title" bind:value={draftTitle} maxlength="90" placeholder="Escribe un título claro y breve" /><label for="community-draft">Comparte algo que te gustaría que otra persona leyera</label><textarea id="community-draft" bind:value={draft} maxlength="420" placeholder="Escribe tu experiencia, una pregunta o algo que te haya ayudado..."></textarea><div class="composer-footer"><small>{draftTitle.length}/90 · {draft.length}/420 · Tu historia se publicará de forma anónima.</small><button class="outline" type="button" on:click={publish} disabled={!draftTitle.trim() || !draft.trim()}>Publicar anónimamente</button></div></div>{/if}
